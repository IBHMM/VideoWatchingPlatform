...
export function WatchList() {
    ...
    const handleClearWatchList = async () => {
        try {
            updateUser({userId : user.id, updatedData : {watchList : []}});
            message.success('WatchList cleared successfully!');
        } catch (error) {
    ...
    return (
        <div className='w-screen flex flex-col items-center justify-center min-h-[500px]'>
            ...
            {
                user?.watchList?.length === 0 ? 
                    <Empty title={"You have no Movie in Watchlist"} /> : 
            ...
            {isError && (
                <p className="text-red-500 mt-4">Failed to update watchList, please try again.</p>
            )}
        </div>
    );
}
